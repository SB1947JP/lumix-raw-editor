...
## Installable as a PWA (desktop + iPad)

- Added a web manifest, app icons and a service worker so the app installs as a standalone app — "Install" in desktop Chrome/Edge, "Add to Home Screen" on iPad — from the one GitHub Pages build. No native toolchain, no new runtime dependency
- The service worker is hand-rolled (like the map and the JPEG metadata writer) rather than pulling in Workbox/vite-plugin-pwa. Runtime caching, no build-time precache manifest to keep in sync: same-origin assets (hashed JS/CSS/wasm, fonts, icons) are cache-first so they load offline and instantly; navigations are network-first so a fresh deploy is picked up online, with the cached shell as the offline fallback. The LibRaw wasm is fetched lazily on first decode, so it enters the cache the first time a photo is opened online. **The cache name carries a version (`sre-cache-v1`) — bump it when cached assets need to be force-refreshed**, since old entries are otherwise served until evicted
- Cross-origin requests (only the OpenStreetMap tiles) are left entirely to the network and never cached — the map needs a connection, and nothing here weakens the app's no-exfiltration stance: the worker only ever reads and stores this origin's own files. Registered from `main.tsx` in production only (a worker caching in dev would fight HMR), with the path and scope carrying the `/seans-raw-editor/` deploy base
- Icons are a gold "SRE" monogram on the app's navy, generated deterministically with a tiny zlib-based PNG encoder (`scratchpad/gen-icons.mjs`) — full-bleed so they double as maskable icons. Verified against a **production** `vite preview` build: manifest parses, service worker registers and controls the page, the shell caches, icons resolve, and the CSP raises no violations
